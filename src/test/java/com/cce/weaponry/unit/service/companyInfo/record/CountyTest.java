...
public class CountyTest extends SpringTxTestCase {
	...
	// 初始化已提交的企业信息
	private void initSubmitedCompanyInfo() {
		for (int i = 0; i < 5; i++) {
			// 注册企业最基本信息
	...
	@Test
	public void testCountyQuery_Wait() {
		initSubmitedCompanyInfo();
		// 待处理审批项
		...
		for (Approval4Register i : approvals.getResult()) {
			...
			// + CompanyUtils.formatDate(i.getCreateDate()) + "   第"
	...
	@Test
	public void testCountyQuary_Complete() {
	...
	// 初始化未通过省级审批的企业信息
	private void initNotPassCityCompanyInfo() {
		for (int i = 0; i < 5; i++) {
			// 注册企业最基本信息
			CompanyInfo company = new CompanyInfo();
		...
		for (int k = 0; k < approvals.getResult().size(); k++) {
			if (k % 2 == 0) {
				...
				// approval4RegisterService.notPassCity(a.getId(), "处理人",
				// "市级审批意见:"
				// + a.getCompanyInfo().getSimpleName());
			}
		}
	...
	// 退回重审
	@Test
	public void testCountyQuery_reExam() {
		initNotPassCityCompanyInfo();
		// 市级退回重审
		System.out.println("市级退回重审");
		Object[] obj = new Object[3];
		Approval4Register app = new Approval4Register();
