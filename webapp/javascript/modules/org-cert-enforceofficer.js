...
api: {
    ...
    create : 'info/enforceOfficer!issuing.action',
    update: 'info/enforceOfficer!issuing.action',
...
var columns = [
    ...
    {header:'证书',dataIndex:'fileId',renderer:linker}
];
...
com.cce.cert.OrgEnforceOfficer=Ext.extend(Ext.Panel,{
...
com.cce.cert.OrgEnforceOfficerMaster = Ext.extend(Ext.grid.GridPanel, {
	  id:'OrgEnforceOfficerMaster',
	  stripeRows: true,
	  loadMask: true,
	  border: false,
	  enableHdMenu: false,
	  ...
	  initComponent : function() {
		    ...
		    // super
		    com.cce.cert.OrgEnforceOfficerMaster.superclass.initComponent.call(this);
	        ...
	        if (!selected) {
		      ...
		      Ext.Msg.confirm('确认删除', '你确定删除该条记录?', function(btn) {
					if (btn == 'yes') {
				        ...
				        {
				        	Ext.getCmp('OrgEnforceOfficerMaster').store.remove(rows[i]);
				        }
				        Ext.getCmp('OrgEnforceOfficerMaster').store.save();
					}
		      });
	  }
});
...
com.cce.cert.EnforceOfficerForm = Ext.extend(Ext.form.FormPanel, {
	title: '执法人员分级管理',
	modal:true,
	iconCls: 'silk-user',
	labelWidth: 100,
  ...
  // private A pointer to the currently loaded record
	...
	initComponent : function() {
	    ...
	    // super
	    com.cce.cert.EnforceOfficerForm.superclass.initComponent.call(this);
	},
    scope:this,	
	...
	{
		var thestore=Ext.getCmp('OrgEnforceOfficerMaster').store;
...
var searchEnforceOfficerInfo = function() {
	...
	Ext.getCmp('OrgEnforceOfficerMaster').store.load({
		params : { 
...
com.cce.cert.EnforceOfficerSearchForm = Ext.extend(Ext.form.FormPanel, {
	title: '档案分级查询',
	modal:true,
	iconCls: 'silk-user',
	labelWidth: 100,
	...
	initComponent : function() {
	    ...
	    // super
	    com.cce.cert.EnforceOfficerSearchForm.superclass.initComponent.call(this);
	},
  ...
  buildUI : function(){
      return [{
			text:"查询",
			scope: this,
			handler:searchEnforceOfficerInfo
	  	}, {
	        text: '关闭',
	        handler: function(btn, ev){
	  			this.fireEvent('afterSearch', this, null);
...
Ext.extend(com.cce.Module, {
	...
	init: function(){
		...
		this.master = new com.cce.cert.OrgEnforceOfficerMaster({ store : this.store });
//		this.detail= new com.cce.cert.OrgEnforceOfficerDetail({ store : this.store });
		this.frame= new com.cce.cert.OrgEnforceOfficer();		
...
//		this.frame.add(this.detail);
	...
	showSearchForm : function(g, store, record){
		...
		var form = new com.cce.cert.EnforceOfficerSearchForm();
	...
	showForm : function(g, store, record){
		...
		var form = new com.cce.cert.EnforceOfficerForm();
		this.newWin = new Ext.Window({
		    title: '执法人员管理',
		    closable:true,
		    width:600,
