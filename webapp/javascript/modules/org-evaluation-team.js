...
var proxy = new Ext.data.HttpProxy({
	api: {
	    read : 'info/evalTeam!list.action',
	    create : 'info/evalTeam!save.action',
	    update: 'info/evalTeam!save.action',
	    destroy: 'info/evalTeam!delete.action'
	}
	});

	var proxy_detail = new Ext.data.HttpProxy({
		api: {
			    read : 'info/evalTeam!findExpertByTeamId.action',
			    create : 'info/evalTeam!save.action',
			    update: 'info/evalTeam!save.action',
			    destroy: 'info/evalTeam!delete.action'
			}
...
com.cce.members.OrgEvalTeam=Ext.extend(Ext.Panel,{
	id:'OrgEvalTeam',
	loadMask: true,
	border: false,
	enableHdMenu: false,
...
com.cce.members.EvalTeamForm = Ext.extend(Ext.form.FormPanel, {
	title: '评价成员基本信息',
	modal:true,
	iconCls: 'silk-user',
	header: false,
    ...
    initComponent : function() {
	    ...
	    // super
	    com.cce.members.EvalTeamForm.superclass.initComponent.call(this);
	},
...
com.cce.members.OrgEvalTeamMaster = Ext.extend(Ext.grid.GridPanel, {
	  id:'com.cce.members.OrgEvalTeamMaster',
	  stripeRows: true,
	  loadMask: true,
	  border: false,
	  enableHdMenu: false,
	  ...
	  initComponent : function() {
		    ...
		    // super
		    com.cce.members.OrgEvalTeamMaster.superclass.initComponent.call(this);
	        ...
	        if (!selected) {
		      ...
		      Ext.Msg.confirm('确认删除', '你确定删除该条记录?', function(btn) {
					if (btn == 'yes') {
				        ...
				        {
				        	Ext.getCmp('com.cce.members.OrgEvalTeamMaster').store.remove(rows[i]);
				        }
				        Ext.getCmp('com.cce.members.OrgEvalTeamMaster').store.save();
					}
		      });
	  }
});
...
com.cce.members.OrgEvalTeamDetail=Ext.extend(Ext.grid.GridPanel ,{
	  stripeRows: true,
	  loadMask: true,
	  border: false,
	  enableHdMenu: false,
	  ...
	  initComponent : function() {
		    ...
		    // super
		    com.cce.members.OrgEvalTeamDetail.superclass.initComponent.call(this);
...
Ext.extend(com.cce.Module, {
	...
	init: function(){
		...
		this.master = new com.cce.members.OrgEvalTeamMaster({ store : this.store });
		this.detail= new com.cce.members.OrgEvalTeamDetail({ store : this.store_detail });
		this.frame= new com.cce.members.OrgEvalTeam();		
	...
	showForm : function(g, store, record){
		...
		Ext.Ajax.request({
			async:false,
			url : 'info/evalTeam!getMemberChoice.action',
			scope:this,
		...
		var form = new com.cce.members.EvalTeamForm({
			record:record,
