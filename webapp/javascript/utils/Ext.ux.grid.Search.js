...
//					fields.push(item.dataIndex);
					if(fieldNames==""){
						fieldNames+="filter_LIKES_";
					}else{
						fieldNames+="_OR_";
					}
					fieldNames+=item.dataIndex;
				}
			});
			if(preFieldNames!=""){
				delete(store.baseParams[preFieldNames]);
				if (store.lastOptions && store.lastOptions.params) {
					delete(store.lastOptions.params[preFieldNames]);
				}
			}
			if(fieldNames!=""){
				store.baseParams[fieldNames]=val;
				// reload store
				store.reload();
				preFieldNames=fieldNames;
			}
		}
